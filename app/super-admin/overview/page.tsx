...
import { Building2, DollarSign, Flame, PhoneCall, PhoneOutgoing } from 'lucide-react';
    ...
    <div className="space-y-8">
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
        ...
        <StatCard
          ...
          delta={`${formatNumber(stats.dialledToday ?? 0)} lead${(stats.dialledToday ?? 0) === 1 ? '' : 's'} dialled`}
          icon={<PhoneOutgoing className="h-4 w-4" />}
        />
        {/* Counted off the sales pipeline, not off a call status nothing set. */}
        <StatCard
          label="Hot Leads"
          value={formatNumber(stats.hotLeads)}
          delta={`${formatNumber(stats.interestedLeads ?? 0)} more interested`}
          accent
