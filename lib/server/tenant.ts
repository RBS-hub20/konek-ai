  ...
  if (!g.__konek2) {
    ...
    g.__konek2 = {
      businesses: [{
        ...
        sales_tenant: false,
        plan_name: 'starter',
        monthly_minutes_included: 300,
        minutes_used_this_month: 0,
        max_call_minutes: 3,
        overage_rate: 0.35,
        minutes_period_start: null,
        trial_phone: null,
        trial_call_id: null,
...
const normalizeBusiness = (b: Record<string, unknown>): Business => ({
  ...
  plan_name: (b.plan_name as string) ?? null,
  monthly_minutes_included: Number(b.monthly_minutes_included ?? 300),
  minutes_used_this_month: Number(b.minutes_used_this_month ?? 0),
  max_call_minutes: Number(b.max_call_minutes ?? 3),
  overage_rate: Number(b.overage_rate ?? 0.35),
  minutes_period_start: (b.minutes_period_start as string) ?? null,
...
export async function updateBusiness(id: string, patch: Partial<Business>): Promise<Business> {
  const allowed = [
    ...
    'plan_name', 'monthly_minutes_included', 'minutes_used_this_month',
    'max_call_minutes', 'overage_rate', 'minutes_period_start',
    'settings',
  ] as const;
...
export async function createBusiness(input: Partial<Business>): Promise<Business> {
  const row = {
    ...
    plan_name: input.plan_name ?? input.plan ?? 'starter',
    monthly_minutes_included: input.monthly_minutes_included ?? 300,
    minutes_used_this_month: 0,
    max_call_minutes: input.max_call_minutes ?? 3,
    overage_rate: input.overage_rate ?? 0.35,
...
const normalizeCall = (r: Record<string, unknown>): CallLog => ({
  ...
  metered_at: (r.metered_at as string) ?? null,
  created_at: (r.created_at as string) ?? nowIso(),
});
